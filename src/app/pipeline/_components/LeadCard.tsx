...
function ageInfo(createdAt: string, today: string) {
...
export function LeadCard({ lead, onClick, agentAvatarMap, isAdmin = false }: LeadCardProps) {
  ...
  const agentLabel = isUnassigned ? t('board.unassigned') : lead.assigned_agent.split('@')[0]
  const docs = (lead.documents ?? []).length
  const notes = (lead.notes ?? []).length
  const age = ageInfo(lead.created_at, t('board.today'))
  ...
  return (
    <div
      ...
      aria-label={`${name}, ${stageName(lead.stage)}`}
    ...
    >
      ...
      <div className={styles.chips}>
        ...
        {lead.move_in && <span className={styles.chip}>{t('board.move', { date: lead.move_in })}</span>}
      </div>
